...
> ### 🎯 快速回答：什么时候能看到新版本？需要手动操作什么？
>
> **你只需要做一件事：合并 PR。** 其他全部自动完成。
>
> ```
> 合并 PR → GitHub Actions 自动部署（1-3分钟）→ 刷新网页看到新版本
> ```
>
> **完整自动部署流程：**
> ```
...
> GitHub Actions 自动触发：
>   ├── 🌀 GitHub Pages 部署（铸渊聊天室网站）
>   ├── 🚀 CD 服务器部署（guanghulab.com）
>   ├── 📚 图书馆目录自动更新
>   ├── 📋 模块文档自动生成
>   └── 📡 Notion 部署通知
>   ↓ 1-3 分钟后
...
- 合并后刷新 `https://qinfendebingshuo.github.io/guanghulab/` 即可
- 服务器部署（guanghulab.com）同样会在合并后自动触发
