...
## Notion Agent 集群集成（Section 7）

### 现状问题

Notion 侧已建立三分身体系（巡检引擎 + 工单引擎 + 接力引擎），共规划 9 条 Pipeline（A→I）。
核心问题：Agent 每次触发的是"机器人脚本"，不是霜砚的核心大脑。
没有大脑醒来的巡检 = 伪巡检。
...
Notion Agent 触发
  ...
  → 铸渊定时监听该数据库（connectors/notion-wake-listener）
