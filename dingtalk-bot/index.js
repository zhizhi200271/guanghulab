...
app.listen(port, () => {
// ========================
// Phase 5 API路由 · Notion数据桥
// ========================
var notionSync = require('./sync/sync-to-notion');

// Notion健康检查
app.get('/api/notion/health', async function(req, res) {
  try {
    var health = await notionSync.healthCheck();
    res.json(health);
  } catch (err) {
...
// Notion连通状态总览
app.get('/api/notion/status', async function(req, res) {
  try {
    var health = await notionSync.healthCheck();
    var configured = notionSync.isConfigured();
    res.json({
      configured: configured,
      online: health.online,
      bot: health.bot_name || null,
      databases: {
